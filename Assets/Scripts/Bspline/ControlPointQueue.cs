...
public class ControlPointQueue
{
    ...
    private CalculationManager calc;
    ...
    public void Initialize(int Np, float dt, CalculationManager calc)
    {
    ...
    //     // float maxSpeed = TPctrl.GetMaxSpeed();
    //     // 最大速度で進んできるときの1フレームの最大移動距離
    //     // float maxMoveDistance = maxSpeed * dt; // 0.01[m]

    //     // 追加した時からどれくらい離れたかで追加するかしないか決める
    //     // float prevTotalS = target.GetPrevTotalS();
    //     float currTotalS = target.getS();
    //     float diff = currTotalS - prevTotalS;

    //     Debug.Log($"prevTotalS{prevTotalS} currTotalS: {currTotalS} diff:{diff}");
    ...
    //         Debug.Log($"past 更新 v1:{v1} 追加するTP: {Tp}");
